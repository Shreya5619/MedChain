...
const DrugsByIntermediary = () => {
  ...
  const handleSearch = async () => {
    if (!intermediary) {
      alert("Please enter a Intermediary name or public key.");
      return;
    }
    ...
    setHasSearched(true);
    try {
      // NOTE: Endpoint might still be 'drugsByManufacturer' if backend logic is reused,
      // but logically it should be checking for any participant in the chain.
      // Assuming backend filters by passed address/name regardless of role if using same endpoint.
  ...
  return (
    <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500">View transactions processed by an intermediary.</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        <div className="relative flex-grow">
          ...
          <input
            placeholder="Enter Intermediary Public Key"
            value={intermediary}
        ...
        >
          {loading ? 'Fetching...' : 'Search Records'}
        </button>
      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div className="md:col-span-2 mt-2">
                  <p className="text-xs text-gray-400 font-mono break-all">
                    TxID: {drug.TransactionDetails.TransactionID}
                  </p>
                </div>
              </div>
            </motion.div>
